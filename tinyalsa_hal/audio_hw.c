...
#define AUDIO_SAMPLERATE_DEFAULT  48000 // 8000
#define AUDIO_PERIODSIZE_DEFAULT  ((AUDIO_SAMPLERATE_DEFAULT / 8000) * 128)
...
struct dev_proc_info MIC_IN_NAME[] =
{
    ...
    {"rockchipes7243", NULL,},
    {"rockchiprk", NULL, },
    {"rockchiprk809co", "es7202",},   // for none AEC
    {"rockchipes7202s", NULL,},   // for AEC added
    {"rockchiprk817co", NULL,},
...
#ifdef FIX_VOL_AND_SEND_VOL_TO_HAL
static int out_set_amp_mute(int card, bool tomute)
{
    bool isdone = false;
    struct mixer *pMixer = NULL;
    struct mixer_ctl *pctl;
    ...
    pMixer = mixer_open_legacy(card);
    if (!pMixer) {
        ALOGE("mMixer is a null point %s %d, CARD = %d",__func__, __LINE__, card);
    } else {
        pctl = mixer_get_control(pMixer,"Playback Mute",0 );
        if(pctl) {
            ALOGD("%s: tomute = %d", __func__, tomute);
            //if(0 == mixer_ctl_set_val(pctl, tomute?-1:0))
            if(0 == mixer_ctl_select(pctl, tomute?"ON":"OFF"))
                isdone = true;
            else {
                ALOGE("%s: mixer_ctl_set_val() error!\n", __func__);
            }
        } else {
            ALOGE("%s: Playback Mute is not exist", __func__);
        }
...
static bool out_set_mute(struct stream_out *out, bool tomute)
{
    struct audio_device *adev = out->dev;
    bool isdone = false;
    ...
    for (int i = 0; i < out->num_configs; ++i) {
        ...
        if (out->devices[i] == AUDIO_DEVICE_OUT_AUX_DIGITAL) {
            if (out == adev->owner[SOUND_CARD_HDMI]) {
                // TODO: HDMI
            }
        } else if (out->devices[i] == AUDIO_DEVICE_OUT_SPEAKER ||
            ...
            out->devices[i] == AUDIO_DEVICE_OUT_BUS) {
                // TODO: SPEAKER
                //ALOGD("%s: start to open mixer of SND_OUT_SOUND_CARD_SPEAKER", __func__);
                isdone = out_set_amp_mute(adev->dev_out[SND_OUT_SOUND_CARD_SPEAKER].card, tomute);
        } else if (out->devices[i] == AUDIO_DEVICE_OUT_SPDIF) {
            // TODO: SPDIF
        }
#ifdef SUPPORT_MULTIAUDIO
        else if (out->devices[i] == VX_ROCKCHIP_OUT_SPDIF0) {
            // TODO: SPDIF0
        } else if (out->devices[i] == VX_ROCKCHIP_OUT_HDMI0) {
            // TODO: HDMI0
        }
#endif
        else if (audio_is_bluetooth_out_sco_device(out->devices[i])) {
            // TODO: BLUETOOTH
        }
    }
...
static int start_output_stream(struct stream_out *out)
{
    ...
    //ALOGD("%s:%d, out->volume[0] = %f, out->volume[1] = %f, mode = %d",__FUNCTION__,__LINE__,out->volume[0], out->volume[1], adev->mode);
    ...
    #ifdef FIX_VOL_AND_SEND_VOL_TO_HAL
    //ALOGD("%s:%d, out->volume[0] = %f, out->volume[1] = %f, mode = %d",__FUNCTION__,__LINE__,out->volume[0], out->volume[1], adev->mode);
    out_set_mute(out, (out->volume[0] <= 0.0f) && (out->volume[1] <= 0.0f) && (adev->mode == AUDIO_MODE_NORMAL));
    #endif
...
static bool is_aec_device(struct stream_in *in, int card, int device)
{
    ...
    if (card == SND_IN_SOUND_CARD_UNKNOWN) {
        ALOGD("mic card is SND_IN_SOUND_CARD_UNKNOWN");
        goto err_out;
    }

    if(NULL != in->dev->voice.streams)
    {
        struct voice_stream *stream = &(in->dev->voice.streams[0]);

        for(int i=0; i < in->dev->voice.stream_num; i++, stream++)
        {
            ...
            if(/*input device*/source->quirks & VOICE_DEVICE_CHANNEL_HAS_REFERENCE)
            {
                if(card == source->snd_card && device == source->snd_pcm)
                {
                    in->aec_handle.quirks = source->quirks;
                    return true;
                }
            }
        }
...
static bool is_aec_available(struct stream_in *in)
{
    #ifdef RK_COMMON_AEC_TEST
    if((/*input device*/in->aec_handle.quirks & VOICE_DEVICE_CHANNEL_HAS_REFERENCE)
       /*&& (AUDIO_MODE_IN_COMMUNICATION == in->dev->mode)*/)
        return true;
    else
        return false;
...
static bool is_aec_running(const struct audio_device *dev)
{
    return dev->aec_running;
}


static void aec_processor_release(struct stream_in *in)
{
    if(/*input device*/in->aec_handle.quirks & VOICE_DEVICE_CHANNEL_HAS_REFERENCE)
    {
...
static int aec_processor_init(struct stream_in *in)
{
    ...
    in->aec_handle.config = in->flags & AUDIO_INPUT_FLAG_FAST ?
                         &pcm_config_in_low_latency_with_ref : &pcm_config_in_with_ref;

    int ret = voice_effect_init(&in->aec_handle.effect, in->aec_handle.config->rate,
...
static int aec_apply_effects(struct stream_in *in)
{
     ...
     */
    ret = voice_effect_process(&in->aec_handle.effect, in->aec_handle.buffer, frames);
    if (ret) {
...
static void dump_in_effectdata(const void* buffer, size_t bytes)
{
    ...
    if(fd == NULL && NULL != buffer && 0 != bytes) {
        char value[PROPERTY_VALUE_MAX];
        property_get("vendor.audio.record.in_effect", value, "0");
        int size = atoi(value);
        if (size > 0) {
            fd=fopen("/data/misc/audioserver/debug_in_effect.pcm","wb+");
            if(fd == NULL) {
                ALOGD("DEBUG open /data/misc/audioserver/debug_in_effect.pcm ,errno = %s",strerror(errno));
            } else {
                ALOGD("dump pcm to file /data/misc/audioserver/debug_in_effect.pcm");
                rest = size*1024*1024;
            }
        }
    }
    ...
    if (fd != NULL) {
        ALOGD("dump in effect pcm %zu bytes", bytes);
        if(NULL == buffer || 0 == bytes)
        {
            fclose(fd);
            fd = NULL;
            ALOGD("record in effect pcmfile closed!");
        }
        else
        {
            ...
            if (0 >= rest) {
                ...
                fd = NULL;
                rest = 0;
                property_set("vendor.audio.record.in_effect", "0");
                ALOGD("TEST record in effect pcmfile end");
            }
        }
    }
}
 ...
 * @returns
 */
static size_t get_aec_input_buffer_size(unsigned int sample_rate,
                                    audio_format_t format,
...
#ifdef RK_NEW_EFFECT_ENABLE
static void dump_in_comm_effectdata(const void* buffer, size_t bytes)
{
    static int rest = 0;
    static FILE* fd = NULL;
...
static void dump_in_rawdata(const void* buffer, size_t bytes)
{
    static int rest = 0;
    static FILE* fd = NULL;
    ...
    if(fd == NULL && NULL != buffer && 0 != bytes) {
        char value[PROPERTY_VALUE_MAX];
        property_get("vendor.audio.record.in_raw", value, "0");
        int size = atoi(value);
        if (size > 0) {
            fd=fopen("/data/misc/audioserver/debug_in_raw.pcm","wb+");
            if(fd == NULL) {
                ALOGD("DEBUG open /data/misc/audioserver/debug_in_raw.pcm ,errno = %s",strerror(errno));
            } else {
                ALOGD("dump pcm to file /data/misc/audioserver/debug_in_raw.pcm");
                rest = size*1024*1024;
            }
        }
    }
    ...
    if (fd != NULL) {
        ALOGD("dump in raw pcm %zu bytes", bytes);
        if(NULL == buffer || 0 == bytes)
        {
            fclose(fd);
            fd = NULL;
            ALOGD("record in raw pcmfile closed!");
        }
        else
        {
            fwrite(buffer,bytes,1,fd);
            ...
            if (0 >= rest) {
                ...
                fd = NULL;
                rest = 0;
                property_set("vendor.audio.record.in_raw", "0");
                ALOGD("TEST record in raw pcmfile end");
            }
        }
    }
}
...
#if defined(BUIDIN_MIC_VOLUME_PROCESS)

#ifndef RK_NEW_EFFECT_ENABLE
static void in_apply_volume(audio_devices_t device, int16_t *buffer, size_t size)
{
    //ALOGD("%s %d: buildin mic device = 0x%x, current device = 0x%x\n", 
    //      __func__, __LINE__, AUDIO_DEVICE_IN_BUILTIN_MIC, in->device);
    
    if(AUDIO_DEVICE_IN_BUILTIN_MIC == device)
    {
        int i;
        int32_t tmp;
        float vol_times = 10.0f;

        for (i = 0; i < size; ++i) {
            tmp = buffer[i] * vol_times;
            if (tmp > 0x7fff) {
                buffer[i] = 0x7fff;
            } else if (tmp < -0x8000) {
                buffer[i] = -0x8000;
...
#define BUILDIN_MIC_MUTE_DURATION_MS    100	//300   // 100 // ms
#define BUILDIN_MIC_FADE_ON_DURATION_MS 100 // 200 // ms
#if (BUILDIN_MIC_FADE_ON_DURATION_MS > 0)
#define BUILDIN_MIC_FADE_START_VOLUME   0.0f
#else
#define BUILDIN_MIC_FADE_START_VOLUME   1.0f
#endif
#define BUILDIN_MIC_FADE_VOLUME_STEP    ((AUDIO_PERIODSIZE_DEFAULT*1000.0f) /(AUDIO_SAMPLERATE_DEFAULT*BUILDIN_MIC_FADE_ON_DURATION_MS))
#endif
#endif
                           ...
                           struct resampler_buffer* buffer)
{
    ...
    if (in->frames_in == 0) {
        ...
        {
            size = pcm_frames_to_bytes(in->pcm, in->aec_handle.config->period_size);
            in->read_status = pcm_read(in->pcm,
                                       (void*)in->aec_handle.buffer, size);

            #ifdef BUIDIN_MIC_VOLUME_FADEIN_ENABLE
            if(0 < in->buidin_mic_mute_samples)
            {
                in->buidin_mic_mute_samples -= in->config->period_size;
                memset(in->aec_handle.buffer, 0, size);
                ...
                if(1.0f <= in->fade_volume)
                    ...
                    int16_t* pdata = in->aec_handle.buffer;
...
static int start_input_stream(struct stream_in *in)
{
    ...
    if (in->device == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
        ...
        if (card != SND_IN_SOUND_CARD_UNKNOWN) {
                ...
                && 0 == aec_processor_init(in))
            {
                ...
                in->buffer = malloc(in->config->period_size * (in->config->channels>>1) *
                                    audio_stream_in_frame_size(&in->stream));
                if (!in->buffer) {
                    aec_processor_release(in);
                    return -ENOMEM;
            ...
            #ifdef BUIDIN_MIC_VOLUME_FADEIN_ENABLE
            in->buidin_mic_mute_samples = ((in->config->rate*BUILDIN_MIC_MUTE_DURATION_MS)/1000);
            #endif
    ...
    #ifdef BUIDIN_MIC_VOLUME_FADEIN_ENABLE
    in->fade_volume = BUILDIN_MIC_FADE_START_VOLUME;
	ALOGD("%s: fade volume step = %f", __func__, BUILDIN_MIC_FADE_VOLUME_STEP);
    #endif
...
static void do_out_standby(struct stream_out *out)
{
    ...
    #if 0//def RK_COMMON_AEC_ENABLE
    if(AUDIO_MODE_IN_COMMUNICATION == adev->mode)
    {
        if(is_aec_running(adev))
            return;
    }
    #endif
...
#endif
        #if 0//def FIX_VOL_AND_SEND_VOL_TO_HAL
        out_set_mute(out, true);
        #endif
                          ...
                          float right)
{
    ...
    last_l = out->volume[0];
    last_r = out->volume[1];
	//------to control speaker amplifer mute/unmute control-------------------------------
	//if((last_l != left) || (last_r != right)) {
	//	if(((left == 0.0f) && (right == 0.0f)) ||
	//	   ((last_l == 0.0f) && (last_r == 0.0f))) {
			bool   isdone;
			bool   tomute = (left <= 0.0f) && (right <= 0.0f) && (adev->mode == AUDIO_MODE_NORMAL);

			ALOGD("last left vol:%f, right vol:%f, new left vol:%f, right vol:%f, adev->mode = %d", last_l, last_r, left, right, adev->mode);

            isdone  = out_set_mute(out, tomute);
			//if(isdone)
			//	ret = 0;
            //else {
                left  = last_l;
...
static void dump_out_data(const void* buffer,size_t bytes)
{
    ...
    if (fd != NULL) {
        ALOGD("dump out pcm %zu bytes", bytes);
        if(NULL == buffer || 0 == bytes)
        {
            fclose(fd);
            fd = NULL;
            ALOGD("record raw pcmfile closed!");
        }
        else
        {
            fwrite(buffer,bytes,1,fd);
...
static void dump_in_data(const void* buffer, size_t bytes)
{
    ...
    if (fd != NULL) {
        ...
        if(NULL == buffer || 0 == bytes)
        {
            fclose(fd);
            fd = NULL;
            ALOGD("record in pcmfile closed!");
        }
        else
        {
            fwrite(buffer,bytes,1,fd);
            ...
            if (0 >= rest) {
                ...
                ALOGD("TEST record in pcmfile end");
            }
        }
    }
}
...
static size_t in_get_buffer_size(const struct audio_stream *stream)
{
    ...
    #ifdef RK_COMMON_AEC_ENABLE
    if(is_aec_running(in->dev)) {
        return get_aec_input_buffer_size(in->requested_rate,
...
static void do_in_standby(struct stream_in *in)
{
    ...
    if(AUDIO_MODE_IN_COMMUNICATION == adev->mode)
    {
        if(is_aec_running(adev))
        {
            ALOGD("%s: aec is still running!", __func__);
            return;
        }
    }
    #endif
        ...
        const struct audio_config *config)
{
    ...
    #ifdef RK_COMMON_AEC_ENABLE
    if(is_aec_running(dev)) {
        return get_aec_input_buffer_size(config->sample_rate, config->format,
...
#ifdef RK_COMMON_AEC_ENABLE
    aec_processor_release(in);
    dump_in_rawdata(NULL, 0);
    dump_in_effectdata(NULL, 0);
#endif

#ifdef RK_NEW_EFFECT_ENABLE
    dump_in_comm_effectdata(NULL, 0);
#endif
        ...
        *patch_handle = *handle;
        ...
        if (adev->mode == AUDIO_MODE_IN_CALL) {
            ...
            if (adev->voice.route != out->devices[0]) {
                ...
                ALOGD("[%s %d] out->devices[0] = %#x", __FUNCTION__, __LINE__, out->devices[0]);
                adev->voice.route = out->devices[0];
                voice_start_incall(adev);
            ...
            #ifdef FIX_VOL_AND_SEND_VOL_TO_HAL
            out_set_mute(out, (out->volume[0] > 0.0f) || (out->volume[1] > 0.0f)? false:true);
            #endif
        }
    } else {
