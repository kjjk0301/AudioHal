...
static void*  thread_start2(void* argv);
...
static void dump_out_data(const void* buffer,size_t bytes, int *size)
{
    ...
    if(fd == NULL) {
        ...
        if(fd == NULL) {
            ALOGD("DEBUG open  error =%p ,errno = %d",fd,errno);
            offset = 0;
        }
    }
...
static int start()
{
    ...
    if (voiceHandle->voice_thread.threadStatus == -1)
        voiceHandle->voice_thread.threadStatus = pthread_create(&voiceHandle->voice_thread.thread, NULL, thread_start2, voiceHandle);
