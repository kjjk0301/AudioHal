...
UpdamplerContext UpSampleHandle2;
...
void audio_effect_process_ASPL(void *in, void *out, size_t frames)
{
	...
	for (int k = 0; k < frames; k++) {	
		TempResample[4*frames+k]=pData[k*4];
	}
	...
	for (int k = 0; k < PCM_CAPTURE_CHANNELS; k++) {
		...
		AsplOutBuf[k] = &in_r_16k[(PCM_CAPTURE_CHANNELS+k)*(frames/3)];
	}
...
//			pData_out[2*i+1] = TempResample[4*frames+i];
