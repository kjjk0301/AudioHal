...
/*
int BPF_Buf[MAX_VAD_FILTER_CHANNELS][NWz+NUM_FRAMES-1]; // need malloc, buffers for VAD filter calculation
...
{
	int i, j;
	vadInst_t *vadInst_p = malloc(sizeof(vadInst_t));
    ssl_vad_Init(vadInst_p);
...
void ssl_vad_Init(void *vadInst)
{
	...
	for (int i = 0; i < MAX_VAD_FILTER_CHANNELS; i++)
	{
		inst->BPF_Buf[i] = (int *)malloc((NWz + NUM_FRAMES - 1) * sizeof(int));
		if (inst->BPF_Buf[i] == NULL)
		{
			perror("ssl_vad_init malloc fail: BPF_Buf col");
			// return;
			// 이미 할당된 메모리 해제
			for (int j = 0; j < i; j++)
			{
	...
	for (j = 0; j < MAX_VAD_CHANNELS; j++)
	{
		inst->g_t_chg[j] = inst->t_chg_min * 20 * inst->d_SNR_ratio;
		inst->g_t_vad[j] = inst->t_chg_min * 20 * inst->d_SNR_vad_ratio;
		inst->g_x_slow_vad[j] = 50000;
		for (i = 0; i < avgsize + vad_idx_total - 1; i++)
		{
			inst->e_hist_Buf[j][i] = 50000;
		}
		inst->g_n_floor[j] = inst->n_floor_min;
		inst->g_state[j] = 0;

		for (m = 0; m < NWz + NUM_FRAMES - 1; m++)
		{
			inst->BPF_Buf[j][m] = 200;
		}
	}

	for (j = 0; j < MAX_VAD_FILTER_CHANNELS; j++)
	{
		for (m = 0; m < NWz + NUM_FRAMES - 1; m++)
		{
			inst->BPF_Buf[j][m] = 200;
...
void ssl_vad_process(vadInst_t *vadInst, short *in, short *ssl_vad_p, short *min_vad_p, short *max_vad_p, int channel)
{
	...
	short *xin = in;
	int *e_hist = &(inst->e_hist_Buf[channel][0]);

	double x_slow_vad = inst->g_x_slow_vad[channel];
	double t_chg = inst->g_t_chg[channel];
	double t_vad = inst->g_t_vad[channel];

	int n_floor = inst->g_n_floor[channel];
	int state = inst->g_state[channel];
	...
	{
		inst->vad_hist1[i] = inst->vad_hist1[i + 1];
		inst->vad_hist2[i] = inst->vad_hist2[i + 1];
	}

	inst->vad_hist1[inst->vad_hold_max - 1] = *ssl_vad_p;

	inst->vad_hist2[inst->vad_hold_max - 1] = MIN(1, inst->vad_hist1[inst->vad_hold_max - 1]);
	for (i = 0; i < (2 - 1); i++)
	{
		inst->vad_hist2[inst->vad_hold_max - 1] = MIN(inst->vad_hist2[inst->vad_hold_max - 1], inst->vad_hist1[inst->vad_hold_max - 1 - (i + 1)]);
	}
	
	int count_pre = MAX(0, inst->vad_hist2[inst->vad_hold_max - 1]);
	for (i = 0; i < inst->vad_hold_max - 1; i++)
	{
		count_pre = MAX(count_pre, inst->vad_hist2[inst->vad_hold_max - 1 - (i + 1)]);
	}

	*max_vad_p = count_pre;

	int count_post = MAX(0, inst->vad_hist2[inst->vad_hold_max - 1]);
	for (i = 0; i < inst->vad_hold_min - 1; i++)
	{
		count_post = MAX(count_post, inst->vad_hist2[inst->vad_hold_max - 1 - (i + 1)]);
	}	
	...
	*min_vad_p = count_post;
	...
	inst->g_x_slow_vad[channel] = x_slow_vad;
	inst->g_t_chg[channel] = t_chg;
	inst->g_t_vad[channel] = t_vad;

	inst->g_n_floor[channel] = inst->n_floor;
	inst->g_state[channel] = state;
}
